    ...
    def addis_systems_mpos__update_partner_data(self, **kw):
        ...
        if http.request.env.user and http.request.env.user.id:
            ...
            for partner in partners:
                ...
                print(partner.name, partner.city)
                return_json.append(single_data)
            return return_json
        else:
            return {'error': 'User authentication failed'}
    ...
    def get_addis_systems_mpos_update_sales_order_data(self, **kw):
        if http.request.env.user and http.request.env.user.id:
            ...
            for sales_order in sales_orders:
                single_data = {}
                single_data["number"] = sales_order.name
                single_data["creation data"] = sales_order.create_date
                single_data["customer"] = sales_order.partner_id.name
                single_data["salesperson"] = sales_order.user_id.name
                single_data["activities"] = sales_order.activity_ids
                single_data["company"] = sales_order.company_id.name
                single_data["total"] = sales_order.amount_total
                single_data["status"] = sales_order.state
                single_data["invoice status"] = sales_order.invoice_status
                single_data["untaxed amount"] = sales_order.amount_untaxed
                single_data["quotation date"] = sales_order.date_order
                single_data["expiration date"] = sales_order.validity_date
                single_data["pricelist"] = sales_order.pricelist_id.name
                single_data["payment terms"] = sales_order.payment_term_id.name
                return_json.append(single_data)
            return return_json
        else:
            return {'error': 'User authentication failed'}
    ...
    def get_addis_systems_mpos_update_purchase_order_data(self, **kw):
        if http.request.env.user and http.request.env.user.id:
            ...
            for purchase_order in purchase_orders:
                single_data = {}
                single_data["reference"] = purchase_order.name
                single_data["vendor"] = purchase_order.partner_id.name
                single_data["company"] = purchase_order.company_id.name
                single_data["buyer"] = purchase_order.user_id.name
                single_data["order deadline"] = purchase_order.date_order
                single_data["activities"] = purchase_order.activity_ids
                single_data["source document"] = purchase_order.origin
                single_data["total"] = purchase_order.amount_total
                single_data["status"] = purchase_order.state
                single_data["untaxed"] = purchase_order.amount_untaxed
                single_data["billing status"] = purchase_order.invoice_status
                single_data["currency"] = purchase_order.currency_id.name
                single_data["expected arrival"] = purchase_order.date_planned
